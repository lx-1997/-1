    ...
    with _connect() as conn:
        conn.execute(
            ...
            INSERT INTO data_sources (
                id, name, source_type, description, status, trust_level, config_json,
...
def list_data_items(
    *,
    ...
    values: list[Any] = []
    if symbol:
        clauses.append("(symbol = ? OR title LIKE ? OR text LIKE ?)")
        like = f"%{symbol}%"
    ...
    if query:
        ...
        for token in tokens[:6]:
            token_clauses.append("(title LIKE ? OR text LIKE ?)")
            like = f"%{token}%"
    ...
    with _connect() as conn:
        rows = conn.execute(
            ...
            {where}
            ORDER BY credibility_score DESC, collected_at DESC, created_at DESC
            LIMIT ?
            """,
            values,
        ).fetchall()

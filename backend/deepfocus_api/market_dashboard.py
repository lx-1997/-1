...
_INSIGHT_TEMPLATES = {
    ...
    "gold": {
        "description": "黄金价格。传统避险资产，与实际利率呈强负相关。",
        # 改相对措辞，避免再被某一年的绝对价位写死过时。
        "historical_context": "黄金处于历史高位区间，长期走势主要由实际利率与避险/央行购金需求驱动。",
        # 分档上移到当前区间（原 2800 天花板使 4000+ 金价恒判"极值"且过时）。
        "interpret": lambda v: (
            "历史极值区 — 避险/通胀对冲需求极高" if v > 4000 else
            "强势 — 实际利率下行+避险需求驱动" if v > 3500 else
            "偏强 — 地缘风险或降息预期支撑" if v > 3000 else
            "中性偏强 — 金价处于较高区间" if v > 2500 else
            "中性 — 金价在均衡区间" if v > 2000 else
            "偏弱 — 风险偏好上升，避险需求减弱"
        ),
        # ⭐金价绝对水平不是干净的方向信号——原 v>2800 恒返 bearish，会给处于历史新高上行趋势的黄金贴"看跌"(明显错)。改为中性。
        "signal": lambda v: "neutral",
        "unit": "$",
    },
    "oil": {

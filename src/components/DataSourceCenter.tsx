...
const categoryMeta: Record<DataSourceCategory, { text: string; color: string }> = {
  market: { text: '行情数据', color: 'purple' },
  earnings: { text: '财报日历', color: 'blue' },
  filing: { text: '公告披露', color: 'geekblue' },
  research: { text: '研报资料', color: 'cyan' },
  sentiment: { text: '舆情社区', color: 'orange' },
  upload: { text: '用户上传', color: 'green' },
  internal: { text: '内部数据', color: 'gold' },
  other: { text: '其他', color: 'default' }
};

const moduleMeta: Record<DataSourceModule, string> = {
  home: '首页',
  stock_detail: '个股详情',
...
const roleMeta: Record<DataSourceRefRole, { text: string; color: string }> = {
  primary: { text: '主数据源', color: 'blue' },
  fallback: { text: '备用', color: 'default' },
  evidence: { text: '证据', color: 'geekblue' },
  signal: { text: '信号', color: 'orange' },
  context: { text: '上下文', color: 'cyan' }
};
...
const defaultRefForm = {
  source_id: '',
  module: 'ai_research' as DataSourceModule,
  role: 'evidence' as DataSourceRefRole,
  notes: ''
};
...
const DataSourceCenter: React.FC<DataSourceCenterProps> = ({ appState }) => {
  ...
  const [refForm, setRefForm] = useState(defaultRefForm);
  const [refFiltersJson, setRefFiltersJson] = useState('{}');
  ...
  const stats = useMemo(() => {
    ...
    const refs = sources.reduce((sum, source) => sum + (source.module_refs?.length || 0), 0);
    const categories = new Set(sources.map(source => source.category)).size;
    return { active, errors, stored, local, remote, tags: tags.length, refs, categories };
  }, [items, sources, tags]);
  ...
  const handleCreate = async () => {
    try {
      const payload: DataSourceCreate = {
        ...
        output_schema: parseJsonObject(outputSchemaJson, '输出结构')
      };
  ...
  const handleSaveRef = async () => {
    if (!refForm.source_id) {
      message.warning('请选择要引用的数据源');
      return;
    }
    setSavingRef(true);
    try {
      await saveDataSourceModuleRef({
        ...refForm,
        filters: parseJsonObject(refFiltersJson, '引用过滤条件'),
        enabled: true
      });
      await loadData();
      message.success('模块引用已保存');
    } catch (error: any) {
      message.error(error?.response?.data?.detail || error.message || '引用保存失败');
    } finally {
      setSavingRef(false);
    }
  };
  ...
  const moduleRefRows = sources.flatMap(source => source.module_refs || []);
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} xl={8}>
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              <Card title={<Space><ApiOutlined />注册数据源</Space>}>
                <Space direction="vertical" size={10} style={{ width: '100%' }}>
                  ...
                  <Select
                    ...
                    options={Object.entries(categoryMeta).map(([value, meta]) => ({ value, label: meta.text }))}
                    placeholder="业务分类"
                  />
                  <Input
                    value={form.url}
                  ...
                  <TextArea rows={3} value={outputSchemaJson} onChange={event => setOutputSchemaJson(event.target.value)} placeholder='输出结构 JSON，例如 {"symbol":"string","title":"string","text":"string"}' />
              ...
              <Card title={<Space><LinkOutlined />模块引用</Space>}>
                <Space direction="vertical" size={10} style={{ width: '100%' }}>
                  <Select
                    showSearch
                    value={refForm.source_id || undefined}
                    onChange={value => setRefForm(prev => ({ ...prev, source_id: value || '' }))}
                  ...
                  <Row gutter={8}>
                    <Col span={12}>
                      <Select
                        value={refForm.module}
                        onChange={value => setRefForm(prev => ({ ...prev, module: value }))}
                        options={Object.entries(moduleMeta).map(([value, label]) => ({ value, label }))}
                      />
                    </Col>
                    <Col span={12}>
                      <Select
                        value={refForm.role}
                        onChange={value => setRefForm(prev => ({ ...prev, role: value }))}
                        options={Object.entries(roleMeta).map(([value, meta]) => ({ value, label: meta.text }))}
                      />
                    </Col>
                  </Row>
                  <TextArea
                    rows={3}
                    value={refFiltersJson}
                    onChange={event => setRefFiltersJson(event.target.value)}
                    placeholder='过滤条件 JSON，例如 {"symbol":"AAPL","lookahead_days":30}'
                  />
                  <Input
                    value={refForm.notes}
                    onChange={event => setRefForm(prev => ({ ...prev, notes: event.target.value }))}
                    placeholder="引用备注"
                  />
                  <Button block icon={<LinkOutlined />} loading={savingRef} onClick={handleSaveRef}>
                    保存模块引用
                  </Button>
                    ...
                    <Table
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          width: 90,
                          render: value => (
                            <Tag color={roleMeta[value as DataSourceRefRole]?.color}>
                              {roleMeta[value as DataSourceRefRole]?.text || value}
                            </Tag>
                          )
                        },
                        {
                        ...
                        },
                        {
                          ...
                          width: 42,
                          render: (_, record) => (
                            <Popconfirm title="删除这个引用？" okText="删除" cancelText="取消" onConfirm={() => handleDeleteRef(record.id)}>
                  ...
                  expandable={{
                    expandedRowRender: record => (
                      <Descriptions size="small" column={1}>
                        ...
                        <Descriptions.Item label="模块引用">
                          {record.module_refs?.length ? (
                            <Space wrap>
                              {record.module_refs.map(ref => (
                                <Tag key={ref.id} color={roleMeta[ref.role]?.color}>
                                  {moduleMeta[ref.module]} · {roleMeta[ref.role]?.text}
                                </Tag>
                              ))}
                            </Space>
                          ) : '-'}
              ...
              <Card title={<Space><FileSearchOutlined />资料资产管理</Space>}>
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                  ...
                  {categoryDistribution.length > 0 && (
                    <Space wrap>
                      <Text type="secondary"><DatabaseOutlined /> 数据源分类</Text>
                      {categoryDistribution.map(item => (
                        <Tag key={item.category} color={item.color}>{item.text} {item.count}</Tag>
                      ))}
                    </Space>
                  )}
                      ...
                      columns={[
                        {
                          ...
                          render: (value, record) => (
                            <Space direction="vertical" size={2} style={{ maxWidth: 520 }}>
                              ...
                              <Space wrap>
                                <Tag color={categoryMeta[record.source_category]?.color}>{categoryMeta[record.source_category]?.text}</Tag>
        ...
        {selectedItem && (
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
            <Descriptions size="small" column={1}>
              ...
              <Descriptions.Item label="分类">{categoryMeta[selectedItem.source_category]?.text}</Descriptions.Item>
        ...
        <Space direction="vertical" size={12} style={{ width: '100%' }}>
          ...
          {editingItem && (
            <Alert
              type="info"
              showIcon
              message={`${editingItem.source_name} · ${categoryMeta[editingItem.source_category]?.text} · ${typeMeta[editingItem.source_type]?.text} · ${new Date(editingItem.collected_at).toLocaleString()}`}
            />
          )}
        </Space>
      </Modal>
